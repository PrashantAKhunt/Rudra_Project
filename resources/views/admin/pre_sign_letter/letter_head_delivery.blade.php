...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-12 col-lg-12 col-sm-12">
            ...
            @endif
            <div class="white-box"> 
                <h4 class="page-title">Signed Letter-head Request</h4>        
                <?php
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.confirm_pre_request') }}" id="confirm_pre_request" method="post" enctype="multipart/form-data">
                            ...
                            <div class="modal-body" id="userTable">
                                ...
                                <div class="form-group "> 
                                    <label>Letter Head Ref Number</label>
                                    <select class="form-control" name="letter_head_ref_no" id="letter_head_ref_no_pre">
                                        <option value="">Select Letter Head Ref Number</option>  
                                    </select> 
                                </div>
                                 <div class="form-group "> 
                    ...
                    <div class="modal-content">
                        ...
                        <form action="{{ route('admin.confirm_pro_request') }}" id="confirm_pro_request" method="post" enctype="multipart/form-data">
                            ...
                            <div class="modal-body" id="userTable">
                                ...
                                <div class="form-group "> 
                                    <label>Letter Head Ref Number</label>
                                    <select class="form-control" name="letter_head_ref_no" id="letter_head_ref_no">
                                        <option value="">Select Letter Head Ref Number</option>    
                                    </select> 
                                </div>
                                <div class="form-group "> 
                                    ...
                                    <select id='letter_head_numbers' name='letter_head_numbers[]' class="select2 m-b-10 select2-multiple" multiple="multiple">
                                        <option value="">Select Letter Head Number</option>
                                    </select>
                                </div>
...
<script>
                    ...
                    "render": function (data, type, row) {
                        ...
                        } else if (row.is_deliver_status == 'Delivered')
                        {
                            if(moment(row.second_approval_datetime).isValid()){
                                return '<b class="text-success">Delivered <br>'  + moment(row.second_approval_datetime).format("DD-MM-YYYY h:mm A")+'</b>';
                            }else{
                                return '<b class="text-success">Delivered</b>';
                            }
                        } else
    ...
    function confirmDelivery(url, status, msg, id) {
        if (msg == "pre")
        {
            $.ajax({
                type : "POST",
                url : "{{url('get_company_pre_letter_head_ref_number')}}",
                data : {
                    "_token" : "{{csrf_token()}}",
                    id : id,
                },
                success:function(data){
                    // console.log(data);
                    $("#letter_head_ref_no_pre").empty();
                    $("#letter_head_ref_no_pre").append(data);
                }
            });
            $("#letter_head_number").select2("val", "");
        ...
        if (msg == "pro")
        {
            $.ajax({
                type : "POST",
                url : "{{url('get_company_letter_head_ref_number')}}",
                data : {
                    "_token" : "{{csrf_token()}}",
                    id : id,
                },
            ...
            $("#letter_head_numbers").select2("val", "");
    ...
    $("#letter_head_ref_no").on('change',function(){
        var letter_head_ref_number = $(this).val();
        $.ajax({
            type : "POST",
            url : "{{url('get_company_letter_head_number')}}",
            data : {
                "_token" : "{{csrf_token()}}",
                letter_head_ref_no : letter_head_ref_number,
            },
            success:function(data){
                // console.log(data);
                $("#letter_head_numbers").empty();
                $("#letter_head_numbers").append(data);
            }
        });
    });
    $("#letter_head_ref_no_pre").on('change',function(){
        var letter_head_ref_no_pre = $(this).val();
        $.ajax({
            type : "POST",
            url : "{{url('get_company_pre_letter_head_number')}}",
            data : {
                "_token" : "{{csrf_token()}}",
                letter_head_ref_no : letter_head_ref_no_pre,
            },
            success:function(data){
                // console.log(data);
