...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-12 col-lg-12 col-sm-12">
            ...
            <div class="white-box"> 
                <h4 class="page-title">Letter-head Request</h4>        
                <?php
                $role = explode(',', $access_rule);
                if (in_array(3, $role)) {
                    ?>
                            ...
                            <div class="modal-body" id="userTable">
                                ...
                                <div class="form-group "> 
                                    ...
                                    <select id='letter_head_number' name='letter_head_number[]' class="select2 m-b-10 select2-multiple" multiple="multiple">
                                        ...
                                        <?php
                                        foreach ($letter_head_data as $key => $letter_head_data_value) {
...
<script>
        ...
        var table = $('#pre_signTable').DataTable({
            ...
            "columns": [
                ...
                {"taregts": 7, "searchable": false, "orderable": false, "render": function (data, type, row) {
                        ...
                        return "<input type='hidden' id='pre_letter_content_input_" + row.id + "' value='" + row.note + "' /><a href='#' class='btn btn-info btn-rounded' onclick='open_letter_content(" + row.id + ",1)' data-toggle='modal' data-target='#letter_content' title='View Content'><i class='fa fa-eye'></i></a>";
                    }
                },
        ...
        var table = $('#signTable').DataTable({
            ...
            "columns": [
                ...
                {"taregts": 7, "searchable": false, "orderable": false, "render": function (data, type, row) {
                        ...
                        return "<input type='hidden' id='letter_content_input_" + row.id + "' value='" + row.note + "' /><a href='#' class='btn btn-info btn-rounded' onclick='open_letter_content(" + row.id + ",0)' data-toggle='modal' data-target='#letter_content' title='View Content'><i class='fa fa-eye'></i></a>";
                    }
                },
                {"taregts": 8, "searchable": true, "render": function (data, type, row) {
                        return moment(row.created_at).format('DD-MM-YYYY h:m a');
    ...
    function open_letter_content(id,type) {
       if(type==1){
        $('#tableBodylatterContent').html($('#pre_letter_content_input_' + id).val());
    }
    else{
