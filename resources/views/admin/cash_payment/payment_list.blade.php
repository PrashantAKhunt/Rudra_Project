                            ...
                            <?php if (!empty($cashApprovealData)) { ?>
                                <?php foreach ($cashApprovealData as $cashData) { ?>
                                    ...
                                    <td>{{ $cashData['payment_options'] }}</td>
                                        ...
                                        <td>
                                            @php
                                                $amount = number_format($cashData['amount'], 2, '.', ',');
                                                $total_amount_words =  app(App\Lib\CommonTask::class)->convert_digits_into_words($cashData['amount']);
                                                $igst_amount = 0;
                                                $cgst_amount = 0;
                                                $sgst_amount = 0;
                                                $main_amount = 0;
                                                if($cashData['igst_amount']){
                                                    $igst_amount = $cashData['igst_amount'];
                                                }
                                                if($cashData['cgst_amount']){
                                                    $cgst_amount = $cashData['cgst_amount'];
                                                }
                                                if($cashData['sgst_amount']){
                                                    $sgst_amount = $cashData['sgst_amount'];
                                                }
                                                $main_amount = $cashData['amount'] - $igst_amount - $cgst_amount - $sgst_amount;
                                                $title = 'Amount = '.$main_amount.', IGST = '.$igst_amount.', CGST = '.$cgst_amount.' , SGST = '.$sgst_amount.'';
                                            @endphp
                                        ...
                                        <td>
                                            @if ($cashData['igst_amount'] && $cashData['igst_amount'] > 0)
                                                    {{number_format($cashData['igst_amount'], 2, '.', ',')}}
                                                    @php
                                                    $total_amount_words =  app(App\Lib\CommonTask::class)->convert_digits_into_words($cashData['igst_amount']);
                                                    @endphp
                                                    <br><b>{{$total_amount_words}}</b>
                                            @else
                                                0.00
                                        ...
                                        <td>
                                            @if ($cashData['cgst_amount'] && $cashData['cgst_amount'] > 0)
                                                    {{number_format($cashData['cgst_amount'], 2, '.', ',')}}
                                                    @php
                                                    $total_amount_words =  app(App\Lib\CommonTask::class)->convert_digits_into_words($cashData['cgst_amount']);
                                                    @endphp
                                                    <br><b>{{$total_amount_words}}</b>
                                            @else
                                                0.00
                                        ...
                                        <td>
                                            @if ($cashData['sgst_amount'] && $cashData['sgst_amount'] > 0)
                                                    {{number_format($cashData['sgst_amount'], 2, '.', ',')}}
                                                    @php
                                                    $total_amount_words =  app(App\Lib\CommonTask::class)->convert_digits_into_words($cashData['sgst_amount']);
                                                    @endphp
                                                    <br><b>{{$total_amount_words}}</b>
                                            @else
                                                0.00
                                        ...
                                        <td>{{ $cashData['note'] }}</td>
                                        <td>{{ $cashData['vender_invoice_no'] }}</td>
                                        ...
                                        <td>
                                            ...
                                            @php
                                            if($cashData['voucher_image']){
                                                $file_path = str_replace('public/','',$cashData['voucher_image']);
                                                $image_link = URL::to('/')."/storage/".$file_path;
                                                echo '<a download="" title="Download Voucher Image" href="'.$image_link.'" class="btn btn-rounded btn-primary"><i class="fa fa-download"></i></a>';
                                            }
                                            @endphp
                                        </td>
                                        <td>
                                          ...
                                          <a onclick="delete_confirm(this);" class="btn btn-danger btn-rounded" href="#" data-href="{{ route('admin.delete_cash_payment',['id'=> $cashData['id'] ]) }}" title="Delete"><i class="fa fa-trash"></i></a>
                                        @endif
            ...
            <div class="modal-dialog">
                <div class="modal-content" id="model_data">
                    ...
                    <div class="modal-body">
                        <form method="post" action="{{ route('admin.approve_cashNewPaymentByAccountant') }}" id="cashPaymentApprove" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                ...
                                <div class="form-group">
                                    <label>Voucher Image Attachment</label>
                    ...
                    "columns": [
                        ...
                        },
                        {
                            ...
                            "render": function(data, type, row) {
                                ...
                                if (role == SuperUser) {
                                    out += '&nbsp;<a onclick="delete_confirm(this);" class="btn btn-danger btn-rounded" href="#" data-href=\'<?php echo url('delete_cash_payment'); ?>/' + row.id + '\'\n\
