...
<div class="container-fluid">
                    ...
                    <table id="user_table" class="table table-striped">
                        ...
                        @foreach($outward_list as $list_data)
                            <tr>
                                ...
                                <td ><span style="display: none;">{{ $list_data->expected_ans_date }}</span>
                                    {{ Carbon\Carbon::parse($list_data->expected_ans_date)->format('d-m-Y') }} </td>
                                @else
                                <td>NA</td>
                                @endif
                                <td ><span style="display: none;">{{ $list_data->created_at }}</span>
                                     {{ Carbon\Carbon::parse($list_data->created_at)->format('d-m-Y H:i:s') }} </td>
                                <td>
                                <a href="{{ route('admin.view_outward_to_inward',['id'=>$list_data->parent_inward_outward_no]) }}" class="btn btn-rounded btn-primary"><i class="fa fa-eye"></i></a>
                                <a href="{{ route('admin.registry_chat',['id'=>$list_data->parent_inward_outward_no]) }}" class="btn btn-rounded btn-success"><i class="fa fa-comments-o"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
    ...
    $(document).ready(function(){
      $('#user_table').DataTable({"order": [[ 6, 'DESC' ]],});
