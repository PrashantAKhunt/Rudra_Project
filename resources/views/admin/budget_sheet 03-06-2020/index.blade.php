        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                var table = $('#policy_table').DataTable({
                    ...
                    "serverSide": true,
                    "responsive": true,
                    //"columnDefs": [ { "type": "date", "targets": 25 } ],
                    "order": [[ 26 , "DESC" ]],
                     //stateSave: true,
                    "ajax": {
                        ...
                        },
                        {
                            ...
                            "render": function(data, type, row) {
                                ...
                                if (row.payment_status == 'Pending') {
                                    return '<span class="label label-rouded label-danger">Pending</span>';
                                } else {
                                    return '<span class="label label-rouded label-info">Done</span>';
                                }
                            }
                        },
                        {"taregts": 25, "searchable": true, "render": function (data, type, row) {
                                return row.payment_date ? moment(row.payment_date).format("DD-MM-YYYY h:mm:ss a") : 'NA';
                            }
                        },
                        {
                            ...
                            "render": function(data, type, row) {
                                ...
                                if (row.status == "Approved" && role == getRole && row.payment_status ==  "Pending") {
                                    out += '<a onclick="payment_done(this);" class="btn btn-success btn-rounded" href="#" data-href="<?php echo url('payment_done_budgetSheet') ?>'+'/'+ row.id +'"  title="Done"><i class="fa fa-check"></i></a>';
            ...
            function payment_done(e) {
                swal({
                    title: "Are you sure you want to done this payment ?",
                    type: "warning",
