                    ...
                    <div class="col-sm-6 col-xs-6">
                        ...
                        <form action="{{route('admin.save_bank_charge_sub_category')}}" id="add_charge" method="post">
                            ...
                            <div class="form-group "> 
                                ...
                                <option value="{{$value['id']}}" @if($value['id'] == $bank_charge['bank_charge_category_id']) selected  @endif>{{$value['title']}}</option>
                                @endforeach
                                </select>
                            </div>
                            <div class="form-group "> 
                                <label>Title <strong> <span class="text-danger">*</span></strong> </label> 
                                <input type="text" class="form-control" name="title" id="title" value="{{$bank_charge['title']}}" /> 
                                <input type="hidden" name="id" value="{{$bank_charge['id']}}">
                            </div>
                            <div class="form-group ">
                                <label>Detail <strong> <span class="text-danger">*</span></strong> </label>
                                <textarea class="form-control" rows="10" name="detail" id="detail">{{$bank_charge['detail']}}</textarea>
                            </div>
