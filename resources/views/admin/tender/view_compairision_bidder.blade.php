		...
		<div class="table-responsive">
			...
			<table class="table table-bordered table-striped" id="bidder_item_table">
				...
				<tbody>
					...
					$new_arr = [];
					@endphp
					<?php $min_bid_amount="";$max_bid_amount=""; $bidder_price_arr=[]; $min_bidder="-1";$max_bidder="-1";$same_price_arr=[];$row_sum1 = 0; ?>
                        @foreach($bidder_item[0]['get_bidder_item'] as $key => $value)
							...
							<tr>
								...
								@foreach($bidder_item as $key1 => $bidder_detail)
                                    ...
                                    if($key==0){
                                        $bidder_item_total_sum[$key1]['totalamt']=0;
                                    }

                                    //echo $bidder_item_total_sum[$key1]['totalamt'];
                                   $bidder_item_total_sum[$key1]['totalamt']=$bidder_item_total_sum[$key1]['totalamt']+$bidder_detail['get_bidder_item'][$key]['total_amount'];
                        ...
                        {{-- For last row total--}}
                        <?php
                            $row_sum1 += $company_items[$key]['total_amount'];
                        ?>

                        @if(count($bidder_item[0]['get_bidder_item']) - 1 == $key)
                            <tr>
                                <td>Total</td>
                                @if($company_items)
								<td>{{$row_sum1}}</td>
                                ...
                                @foreach($bidder_item_total_sum as $key3 => $final_total_bidder_amt)
                                    <td>{{$final_total_bidder_amt['totalamt']}}</td>
								@endforeach
                            </tr>
                        @endif
                        @endforeach
...
$(document).ready(function(){
	var table = $("#bidder_item_table").DataTable({
		'columnDefs': [
		 {
		      "targets": 0,
