                            ...
                            <?php if (!empty($vehicle_maintenance_history)) { ?>
                                <?php foreach ($vehicle_maintenance_history as $key => $aprovals) { ?>
                                    <tr>
                                        ...
                                        <td>{{ date('d-m-Y',strtotime($aprovals->maintenance_date)) }}</td>
                                        <td>{{ date('d-m-Y',strtotime($aprovals->received_date)) }}</td>
                                        <td>
        ...
        <script>
            ...
            "processing": true,
                    ...
                    {"taregts": 1, "searchable": true, "render":function(data, type, row){
                            return moment(row.maintenance_date).format('DD-MM-YYYY');
                    }},
                    {"taregts": 1, "searchable": true, "render":function(data, type, row){
                            return moment(row.received_date).format('DD-MM-YYYY');
                    }},
                    {"taregts": 10,
                    ...
                    {"taregts": 14, "searchable": false, "orderable": false,
                            ...
                            out +='<button class="btn btn-rounded btn-primary" onclick="get_vehicle_maintenance_files('+row.id+')" data-toggle="modal" data-target="#vehicleMaintenanaceFilesModel"><i class="fa fa-eye"></i></button>'
