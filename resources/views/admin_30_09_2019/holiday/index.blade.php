@extends('layouts.admin_app')

@section('content')
<div class="container-fluid">
    <div class="row bg-title">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title">{{ $page_title }}</h4>
        </div>
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">

            <ol class="breadcrumb">
                <li><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                <li><a href="#">{{ $page_title }}</a></li>
            </ol>
        </div>        
    </div>
<div class="row">
    <div class="col-md-12 col-lg-12 col-sm-12">
        @if (session('error'))
        <div class="alert alert-danger alert-dismissable">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            {{ session('error') }}
        </div>
        @endif
        @if (session('success'))
        <div class="alert alert-success alert-dismissable">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            {{ session('success') }}
        </div>
        @endif
        <div class="white-box">
            <a href="{{ route('admin.add_holiday') }}" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Holiday</a>
            <p class="text-muted m-b-30"></p>
            <br>
            <div class="table-responsive">
                <table id="holiday_table" class="table table-striped">
                    <thead>
                        <tr>
                            <th>Title</th>
                            <th>Date</th>
                            <th>Is Optional</th>
							<th>Status</th>
                            <th>Edit</th>
                            <th>Delete</th>
						</tr>
                    </thead>
                    <tbody>
                    </tbody>
                </table>
            </div>
        </div>            
    </div>    
@endsection

@section('script')		
<script>
    $(document).ready(function () {
        var table = $('#holiday_table').DataTable({
            "processing": true,
            "serverSide": true,
            "responsive": true,
            "ajax": {
                url: "<?php echo route('admin.get_holiday_list'); ?>",
                type: "GET",
            },
            "columns": [
				{"taregts": 0, "searchable": true, "data": "title"},
                {"taregts": 1, "searchable": true, "render":function(data,type,row){    
                        if(row.start_date == row.end_date){
                            return moment(row.start_date).format("DD/MM/YYYY");
                        }else{
                            return moment(row.start_date).format("DD/MM/YYYY")+" - "+moment(row.end_date).format("DD/MM/YYYY");
                        }                        
                    }
                },                
                {"taregts": 2,
                    "render": function (data, type, row) {
                        var id = row.id;
                        var out = '';
                        if(row.is_optional == 2){
                            out += '<span class="btn btn-success" title="Change Status">Yes</span>';
                        }else{
                            out += '<span class="btn btn-danger" title="Change Status">No</span>';
                        }
                        return out;
                    }
                },
                {"taregts": 3,
                    "render": function (data, type, row) {
                        var id = row.id;
                        var out = '';
                        if(row.status=='Enabled'){
                        out += '<a href="<?php echo url('admin/change_holiday_status') ?>'+'/'+id+'/Disabled'+'" class="btn btn-success" title="Change Status">'+row.status+'</a>';
                        }
                        else{
                        out += '<a href="<?php echo url('admin/change_holiday_status') ?>'+'/'+id+'/Enabled'+'" class="btn btn-danger" title="Change Status">'+row.status+'</a>';    
                        }
                        return out;
                    }
                },
				{"taregts": 4, "searchable": false, "orderable": false,
                    "render": function (data, type, row) {
                        var id = row.id;
                        return '<a href="<?php echo url("edit_holiday") ?>'+'/'+id+'" class="btn btn-primary btn-rounded"><i class="fa fa-edit"></i></a>';
                    }
                },
				{"taregts": 5, "searchable": false, "orderable": false,
                    "render": function (data, type, row) {
                        var id = row.id;
                        return '<a href="<?php echo url("delete_holiday") ?>'+'/'+id+'" class="btn btn-primary btn-rounded"><i class="fa fa-times"></i></a>';
                    }
                },
            ]
        });
    })
</script>
@endsection