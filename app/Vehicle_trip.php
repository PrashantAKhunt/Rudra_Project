...
{
    protected $table="vehicle_trip";


    public function assets() {
        return $this->hasOne('App\Asset', 'id', 'asset_id');
    }
    public static function get_close_list_datatable_ajax($table,$datatable_fields, $conditions_array, $getfiled, $request, $join_str = array(),$conditions_array1=[],$where_date=[]) {
        $output = array();
        $data = DB::table($table)
                ->select($getfiled);
        ...
        if (!empty($conditions_array1)) {
            $data->where($conditions_array1);
        }
        if(Auth::user()->role==config('constants.SuperUser')) {
        	$data->orWhere(['trip_user_id'=>Auth::user()->id]);	
        }
