...
class AssetAccess extends Model
{
    protected $table="asset_access";

    public function asset() {
        return $this->hasOne('App\Asset', 'id', 'asset_id')->select(['id','name']);
    }
    public function user() {
        return $this->hasOne('App\User', 'id', 'asset_access_user_id')->select(['id','name']);
        
    }

    public static function get_list_datatable_ajax($table,$datatable_fields, $conditions_array, $getfiled, $request, $join_str = array(),$where_date=[],$or_where=[]) {
        $output = array();
        $data = DB::table($table)
                ->select($getfiled);
