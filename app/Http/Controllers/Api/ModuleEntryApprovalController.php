...
class ModuleEntryApprovalController extends Controller
{
    ...
    public function api_entry_approval_count(Request $request)
    {
        ...
        $response_data['sender_tds_section_entry_count'] = $sender_tds_section_entry_count;
    ...
    #------------------ TDS Section Type
    public function api_sender_tds_section_entry_approval_list(Request $request){
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
        ]);
        ...
        $loggedin_user_data = User::where('id', $request_data['user_id'])->get(['users.id', 'users.name', 'users.role']);
        $select_fields = ['tds_section_type.*'];
        $tds_section_type_entry = TdsSectionType::where('status', 'Disabled')
        ->where('is_approved', 0)
        ->get($select_fields);
        if ($tds_section_type_entry->count() == 0) {
            return response()->json(['status' => false, 'msg' => "No Record found.", 'data' => []]);
        }

        $response_data['tds_section_type_entry_list'] = $tds_section_type_entry;
        return response()->json(['status' => true, 'msg' => "Record found.", 'data' => $response_data]);
    }

    public function api_accept_tds_section_entry(Request $request){
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
        ...
        $request_data = $request->all();
        $id = $request_data['tds_section_id'];
        $response_data = [];

        if (TdsSectionType::where('id', $id)->update(['status' => 'Enabled', 'is_approved' => 1])) {

            return response()->json(['status' => true, 'msg' => "TDS section entry successfully Approved", 'data' => []]);
        }
        return response()->json(['status' => false, 'msg' => "Oops, Something went wrong!", 'data' => []]);
    }

    public function api_reject_tds_section_entry(Request $request){
        $validator = Validator::make($request->all(), [
            'user_id' => 'required',
            'tds_section_id' => 'required'
        ]);
        ...
        $id = $request_data['tds_section_id'];
        $response_data = [];
        $entry = TdsSectionType::where('id', $id)->first();
        if ($entry['user_id']) {
            $mail_list = User::where('id', $entry['user_id'])->pluck('email')->toArray();
        }
        // dd($mail_list);
        if (TdsSectionType::where('id', $id)->delete()) {
            if ($entry['user_id']) {
                $mail_data = [
                    'title' => $entry['section_type'],
                    'email_list' => $mail_list
                ];
                $this->common_task->apiTdsSectionEntryRejected($mail_data);
            }
            return response()->json(['status' => true, 'msg' => "TDS Section Entry successfully Rejected", 'data' => []]);
        }
        return response()->json(['status' => false, 'msg' => "Oops, Something went wrong!", 'data' => []]);
    }
}
