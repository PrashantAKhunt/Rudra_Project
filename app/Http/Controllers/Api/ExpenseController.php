...
class ExpenseController extends Controller
{
    ...
    public function add_expense(Request $request)  //change
    {
        ...
        if(strtotime($request_data['expense_date'])<strtotime(date('Y-m-d'))){
            return response()->json(['status' => false, 'msg' => "You can not add the past date entry.", 'data' => [], 'error' => config('errors.general_error.code')]);
        }
    ...
    public function delete_expense(Request $request)
    {
        ...
        // User Action Log
            $expense_code = Employee_expense::where('id', $request_data['expense_id'])->value('expense_code');
            $action_data = [
                'user_id' => $request_data['user_id'],
    ...
    public function approve_reject_expense(Request $request)
    {
        ...
        // User Action Log
        $expense_code = Employee_expense::where('id', $request_data['expense_id'])->value('expense_code');
        $expense_amount = Employee_expense::where('id', $request_data['expense_id'])->value('amount');
        $action_data = [
            'user_id' => $request_data['user_id'],
            'task_body' => "Expense code ".$expense_code." ".$expense_status."<br> Amount: ".$expense_amount,
            'created_ip' => \Request::ip(),
        ];
        $this->user_action_logs->action($action_data);
    ...
    public function add_driver_expense(Request $request)
    {
        ...
        $driver_expense_arr = [
            ...
            'reambance_type' => $request_data['reambance_type'],
            'created_at' => date('Y-m-d H:i:s'),
            'created_ip' => $request->ip(),
            'updated_at' => date('Y-m-d H:i:s'),
            'updated_ip' => $request->ip(),
        ...
        if($request_data['reambance_type'] == "No")
        {
            $driver_expense_arr['first_approval_status'] = 'Approved';
            $driver_expense_arr['first_approval_id'] = $request_data['user_id'];
            $driver_expense_arr['second_approval_status'] = 'Approved';
            $driver_expense_arr['second_approval_id'] = $request_data['user_id'];
            $driver_expense_arr['third_approval_status'] = 'Approved';
            $driver_expense_arr['third_approval_id'] = $request_data['user_id'];
            $driver_expense_arr['status'] = 'Approved';
        }

        $expense_id =Driver_expense::insertGetId($driver_expense_arr);
            ...
            if ($main_km) {
                ...
                if ($vehicle_main) {
                    ...
                    if ($check_periodic_km > $vehicle_main['periodic_maintenance_km']) {
                        // notification fire
                        $this->notification_task->periodicMaintenanceNotify([$request_data['user_id']]);
                    }
                }
            }
        }
    ...
    public function edit_driver_expense(Request $request)
    {
        ...
        if ($request->input('reambance_type') == "No") {
            PeriodicMaintenanceKilometer::insert([
                'driver_expense_id' => $request_data['driver_expense_id'],
                'periodic_maintenance_kilometer' => $request->get('meter_reading'),
                'asset_id' => $request->input('vehicle_type'),
                'created_ip' => $request->ip(),
                'updated_ip' => $request->ip(),
